...
const DriverOrdersPage = () => {
    ...
    const driverOrdersApi = useApi<Order[]>(`${import.meta.env.VITE_API_URL}order/waiting-orders-restaurant?restaurantId=`,token)
    const deliverOrderApi = useApi<Order |null>(`${import.meta.env.VITE_API_URL}order/deliver-order?`, token)
    ...
    const deliverOrder = async (orderId: number, restaurantId: number) => {
        const res = await deliverOrderApi.post(`orderId=${orderId}&restaurantId=${restaurantId}`, {})
            ...
            DriverOrders
            {
                ...
                driverOrdersApi.data.data.map((order)=>(
                    <div>
                        ...
                        <button onClick={()=> deliverOrder(order.id, order.restaurantId)}>deliver</button>
                    </div>
                ))
            }
            {deliverToast && (
